...
public class Iec102SendRequest {
    ...
    public void sendLinkIncrease() throws Exception {
        ...
        while (!isEnd) {
            if (controlDomainStr.equals("5A")) {
                controlDomain_m = getReturnFrame(controlDomain_5A);
                if(controlDomain_m==null){
                    return;
                }else if(controlDomain_m.getAcd() == 0){
                    isEnd = true;
                }
            } else {
                controlDomain_m = getReturnFrame(controlDomain_7A);
                if(controlDomain_m==null){
                    return;
                }else if(controlDomain_m.getAcd() == 0){
                    isEnd = true;
    ...
    private ControlDomain_M getReturnFrame(ControlDomain_C controlDomain) throws Exception {
        ...
        AsduHead asduHead = returnFrame.getAsdu().getAsduHead();
            ...
            return (ControlDomain_M) returnFrame.getControlDomain();
        } catch (Exception e){
