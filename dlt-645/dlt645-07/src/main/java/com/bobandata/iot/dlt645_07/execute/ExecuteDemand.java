...
public class ExecuteDemand implements ITaskExecutor {
    private static final Logger logger = LoggerFactory.getLogger(ExecuteDemand.class);
    private TaskParam taskParam;
    private RxtxSocketAdapter channel;
    private String meterCode;
    private HisDemand hisDemand;
    ...
    public void execute(ITaskParam iTaskParam) throws Exception {
        ...
        for (String meterCode : meterCodes) {
            ...
            this.hisDemand = new HisDemand();

            this.obtainBean(DataTypeConst.POSITIVE_ACTIVE_MAX_DEMAND_AND_TIME);
            this.obtainBean(DataTypeConst.POSITIVE_REACTIVE_MAX_DEMAND_AND_TIME);
            this.obtainBean(DataTypeConst.REVERSE_ACTIVE_MAX_DEMAND_AND_TIME);
            this.obtainBean(DataTypeConst.REVERSE_REACTIVE_MAX_DEMAND_AND_TIME);

            //封装数据结果
            hisDemand.setMeterId(Long.parseLong(meterCode));
            hisDemand.setOccurTime(new Timestamp((new Date()).getTime()));
            iDatas.add(hisDemand);
        }
    }

    @Override
    public void save() {
        HttpDataManager.save(HisDemand.class,iDatas);
    }
        ...
        if (funCode == FunCodeConst.RES_NORMAL) {
            ...
            switch (dataTypeConst) {
                case POSITIVE_ACTIVE_MAX_DEMAND_AND_TIME:
                    hisDemand.setPapRawValue(Double.parseDouble(dataReturnArea.getDataValue().getValue()));
                    hisDemand.setPapValue(Double.parseDouble(dataReturnArea.getDataValue().getValue()) * 0.0001);
                    hisDemand.setPapTime(dataReturnArea.getDataValue().getTimestamp());
                    break;
                case POSITIVE_REACTIVE_MAX_DEMAND_AND_TIME:
                    hisDemand.setRapRawValue(Double.parseDouble(dataReturnArea.getDataValue().getValue()));
                    hisDemand.setRapValue(Double.parseDouble(dataReturnArea.getDataValue().getValue()) * 0.0001);
                    hisDemand.setRapTime(dataReturnArea.getDataValue().getTimestamp());
                    break;
                case REVERSE_ACTIVE_MAX_DEMAND_AND_TIME:
                    hisDemand.setPrpRawValue(Double.parseDouble(dataReturnArea.getDataValue().getValue()));
                    hisDemand.setPrpValue(Double.parseDouble(dataReturnArea.getDataValue().getValue()) * 0.0001);
                    hisDemand.setPrpTime(dataReturnArea.getDataValue().getTimestamp());
                    break;
                case REVERSE_REACTIVE_MAX_DEMAND_AND_TIME:
                    hisDemand.setRrpValue(Double.parseDouble(dataReturnArea.getDataValue().getValue()));
                    hisDemand.setRrpValue(Double.parseDouble(dataReturnArea.getDataValue().getValue()) * 0.0001);
                    hisDemand.setRrpTime(dataReturnArea.getDataValue().getTimestamp());
                    break;
                default:
